...
def gateway(
    ...
    agent = AgentLoop(
        ...
        tokens_window=config.agents.defaults.tokens_window,
        ...
        self_evolution_config=config.tools.self_evolution,
    )
...
def status():
    ...
    if config_path.exists():
        ...
        # Token quota info
        console.print("\n[bold]Token Quota:[/bold]")
        console.print(f"  tokensWindow: {config.agents.defaults.tokens_window:,}")
        ...
        # Session stats and memory status
        import asyncio
        from nanobot.session.store import PersistenceManager
        ...
        async def show_session_info():
            ...
            if db_path.exists():
                ...
                # Get current CLI session stats
                async with db.execute("""
                    SELECT COUNT(*) as msg_count
                    FROM messages m
                    JOIN sessions s ON m.session_id = s.id
                    WHERE s.key = 'cli:direct'
                """) as cursor:
                    row = await cursor.fetchone()
                    msg_count = row["msg_count"] if row else 0

                # Get total token usage for CLI session (approximate)
                token_usage = msg_count * 200  # rough estimate

                console.print("\n[bold]Current Session (cli:direct):[/bold]")
                console.print(f"  Messages: {msg_count}")
                console.print(f"  Est. tokens: ~{token_usage:,}")
                ...
                if memory_path.exists():
                    ...
                    console.print(f"  MEMORY.md: [green]✓[/green] ({size:,} bytes)")
                else:
                    console.print(f"  MEMORY.md: [dim]not exists[/dim]")

                if history_path.exists():
                    size = history_path.stat().st_size
                    console.print(f"  HISTORY.md: [green]✓[/green] ({size:,} bytes)")
                else:
                    console.print(f"  HISTORY.md: [dim]not exists[/dim]")
...
evolution_app = typer.Typer(help="Manage self-evolution proposals")
app.add_typer(evolution_app, name="evolution")


def _evolution_log_path(workspace: Path) -> Path:
    return workspace / "evolution" / "proposals.jsonl"


def _read_evolution_records(path: Path) -> list[dict]:
    if not path.exists():
        return []
    records: list[dict] = []
...
def _append_evolution_record(path: Path, record: dict) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("a", encoding="utf-8") as f:
        f.write(json.dumps(record, ensure_ascii=False) + "\n")


def _collect_proposals(records: list[dict]) -> list[dict]:
...
async def _apply_proposal_actions(workspace: Path, actions: list[dict]) -> list[dict]:
    ...
    runtime = WorkspaceToolRuntime(workspace, registry)
    runtime.reload()

    skill_manager = SkillManagerTool(workspace=workspace)
    tool_manager = ToolManagerTool(workspace=workspace, reload_callback=runtime.reload)
    ...
    for action in actions:
        ...
        if kind == "skill_update":
            out = await skill_manager.execute(
                action="update",
                name=name,
...
def evolution_list(
    limit: int = typer.Option(10, "--limit", "-n", help="Max proposals to show"),
):
    """List self-evolution proposals."""
    from nanobot.config.loader import load_config

    config = load_config()
    path = _evolution_log_path(config.workspace_path)
    proposals = _collect_proposals(_read_evolution_records(path))

    if not proposals:
        console.print("No evolution proposals found.")
        return
    ...
    for p in proposals[: max(1, limit)]:
        ...
        table.add_row(
            ...
            f"{float(p.get('confidence', 0.0)):.2f}",
            str(len(p.get("actions") or [])),
            summary,
        )
...
def evolution_show(
    proposal_id: str = typer.Argument(..., help="Proposal ID (e.g. ev-202603...)"),
):
    """Show details of one self-evolution proposal."""
    from nanobot.config.loader import load_config
    ...
    console.print(f"[bold]Proposal[/bold]: {proposal_id}")
    console.print(f"- status: {proposal.get('status', 'pending')}")
    console.print(f"- confidence: {float(proposal.get('confidence', 0.0)):.2f}")
    console.print(f"- created_at: {proposal.get('created_at', '-')}")
    if proposal.get("decided_at"):
        console.print(f"- decided_at: {proposal.get('decided_at')}")
    if proposal.get("decision_reason"):
        console.print(f"- reason: {proposal.get('decision_reason')}")
    ...
    if auto_applied:
        console.print("[bold]Auto Applied[/bold]:")
        for item in auto_applied:
            line = str(item.get("result", "")).splitlines()[0]
            console.print(f"- {item.get('kind')}:{item.get('name')} -> {line}")
...
def evolution_approve(
    ...
    """Approve and apply a proposal's actions."""
    ...
    actions = proposal.get("actions") or []
    results = asyncio.run(_apply_proposal_actions(config.workspace_path, actions))
    ...
    _append_evolution_record(path, decision)

    console.print(f"[green]Approved:[/green] {proposal_id} ({status})")
    for r in results:
        line = str(r.get("result", "")).splitlines()[0]
        console.print(f"- {r.get('kind')}:{r.get('name')} -> {line}")
...
def evolution_reject(
    ...
    """Reject a proposal without applying actions."""
    from datetime import datetime
    from nanobot.config.loader import load_config
